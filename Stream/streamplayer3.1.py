...
def load_stations_ods(stations_file="stations.ods"):
    if not os.path.exists(stations_file):
        logging.error(f"Station file not found: {stations_file}")
        sys.exit(1)
    try:
        df = pd.read_excel(stations_file, engine='odf', index_col=0)
        df.index = df.index.astype(str).str.zfill(2)  # Ensure IDs are two-digit strings
        return df.to_dict(orient='index')
    except Exception as e:
        logging.error(f"Error reading ODS file {stations_file}: {e}")
        sys.exit(1)
...
def main():
    ...
    all_stations = load_stations_ods()
